...
class AlumniController extends Controller
{
    public function index(Request $request): Response
    {
        return Inertia::render('Alumni/Index', [
            'alumni' => User::query()->whereHas('profile', fn ($q) => $q->where('member_status', 'alumni')->where('alumni_visible', true))->with('profile')->orderBy('name')->get(['id', 'name', 'email']),
    ...
    public function requestMentor(Request $request): RedirectResponse
    {
        ...
        $mentor = User::query()->whereKey($data['mentor_id'])->whereHas('profile', fn ($q) => $q->where('member_status', 'alumni')->where('mentor_available', true)->where('alumni_visible', true))->firstOrFail();
